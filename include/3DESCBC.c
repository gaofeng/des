...
DataChunk DESCBC3Encrypt(DataChunk messageChunk, Block key1, Block key2)
{
	...
	for (i = 0; i < messageChunk.iLength / 8; i++) {
		...
		cipher = DES3Encrypt(blockXOR(message, cipher, 64), k1, k2);
	}
	memcpy(cipherChunk.block + 8 * i, cipher, 8);
	free(message);
...
DataChunk DESCBC3Decrypt(DataChunk cipherChunk, Block key1, Block key2)
{
	...
	for (i = 0; i < messageChunk.iLength / 8; i++) {
		...
		message = blockXOR(cipher, DES3Decrypt(cipherChunk.block + 8 * (i + 1), k1, k2), 64);
